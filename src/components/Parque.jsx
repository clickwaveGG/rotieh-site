...
export default function Parque() {
  return (
    <section id="parque" className="border-y border-teal/15 bg-cream-2/60">
      <div className="mx-auto max-w-6xl px-6 py-20 md:py-28">
        ...
        <div className="mt-14 grid border-t border-teal/15 md:grid-cols-4">
          {ESTRUTURA.map((g, idx) => (
            <div
              key={g.grupo}
              className="border-b border-teal/15 py-7 md:border-b-0 md:border-r md:px-7 md:first:pl-0 md:last:border-r-0"
            >
              <div className="flex items-baseline gap-3">
                <Num n={idx + 1} />
                <h3 className="text-sm font-semibold uppercase tracking-[0.15em] text-teal">
                  {g.grupo}
                </h3>
              </div>
              <ul className="mt-4 space-y-2.5">
                {g.itens.map((item) => (
                  <li key={item} className="text-[13px] leading-relaxed text-teal/65">
                    {item}
                  </li>
                ))}
              </ul>
        ...
        <div className="relative mt-6 overflow-hidden">
          <img
            ...
            loading="lazy"
          />
          <div className="absolute inset-x-0 bottom-0 flex flex-wrap gap-x-6 gap-y-2 bg-gradient-to-t from-teal-deep/80 to-transparent px-6 pb-5 pt-16 md:px-8">
